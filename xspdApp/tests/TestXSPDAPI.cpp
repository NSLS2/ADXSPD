...
TEST_F(TestXSPDAPI, TestGetVersionInfoAfterInitialization) {
    this->mockXSPDAPI->MockInitialization("device123");
...
TEST_F(TestXSPDAPI, TestGetDeviceAtIndex) {
    this->mockXSPDAPI->MockInitialization("device123");
    this->mockXSPDAPI->MockRepeatedGetRequest(this->mockXSPDAPI->expectedDeviceUri, this->mockXSPDAPI->sampleDeviceList);
...
TEST_F(TestXSPDAPI, TestAPIInitInvalidDeviceId) {
    InSequence seq;
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedApiUri, this->mockXSPDAPI->sampleApiResponse);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri, this->mockXSPDAPI->sampleDeviceList);
...
TEST_F(TestXSPDAPI, TestAPIInitNoDeviceInfo) {
    ...
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedApiUri, this->mockXSPDAPI->sampleApiResponse);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri, this->mockXSPDAPI->sampleDeviceList);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri + "/device123/variables?path=info", json());
...
TEST_F(TestXSPDAPI, TestAPIInitNoDetectors) {
    InSequence seq;
    json modifiedInfoResp = this->mockXSPDAPI->sampleInfoVar;
    modifiedInfoResp["value"]["detectors"] = json::array();
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedApiUri, this->mockXSPDAPI->sampleApiResponse);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri, this->mockXSPDAPI->sampleDeviceList);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri + "/device123/variables?path=info", modifiedInfoResp);
    EXPECT_THAT([&]() { this->mockXSPDAPI->Initialize("device123"); },
                testing::ThrowsMessage<std::runtime_error>(
...
TEST_F(TestXSPDAPI, TestAPIInitNoDataPorts) {
    InSequence seq;
    json modifiedDeviceInfo = this->mockXSPDAPI->sampleDeviceInfo;
    modifiedDeviceInfo["system"]["data-ports"] = json::array();
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedApiUri, this->mockXSPDAPI->sampleApiResponse);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri, this->mockXSPDAPI->sampleDeviceList);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri + "/device123/variables?path=info",
                         this->mockXSPDAPI->sampleInfoVar);
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedDeviceUri + "/device123", modifiedDeviceInfo);
    EXPECT_THAT([&]() { this->mockXSPDAPI->Initialize("device123"); },
                testing::ThrowsMessage<std::runtime_error>(
...
TEST_F(TestXSPDAPI, TestGetValidEndpoint) {
    ...
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->expectedApiUri + "/v1/test-endpoint", json{{"key", "value"}});
    json response = this->mockXSPDAPI->Get("test-endpoint");
...
TEST_F(TestXSPDAPI, TestGetIntDetectorVar) {
    ...
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->device123VarUri + "lambda/testIntVar", response);
...
TEST_F(TestXSPDAPI, TestSettingHighThresholdFirstThrowsError) {
    ...
    this->mockXSPDAPI->MockGetRequest(this->mockXSPDAPI->device123VarUri + "lambda/thresholds",
