...
class XSPDAPITest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        ...
        EXPECT_CALL(*mockXSPDAPI, GetApiVersion())
    ...
    json sampleDeviceList = json::array({
        { {"id", "device123"} },
        { {"id", "device456"} }
    });
    json sampleDeviceInfo = {
        {"system", {
            {"detectors", json::array({
                { {"id", "lambda"}, {"n_modules", 2} }
            })},
            {"data-ports", json::array({
                { {"id", "port01"}, {"ip", "192.168.1.1"}, {"port", 1234} },
    ...
    json sampleModuleInfo = {
        {"module", "module0"},
        {"firmware", "v1.0"},
        {"chip_ids", json::array({"chipA", "chipB"})}
    };

    json sampleInfoVar = {
        {"libxsp version", "1.2.3"},
        {"detectors", json::array({
            { {"detector-id", "lambda"}, {"modules", this->sampleModuleInfo} }
        })}
    };

    std::map<std::string, std::string> sampleVarResponse = {
        {"status", "1"},
        {"message", "success"}
    };
};
