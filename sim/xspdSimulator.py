...
parser.add_argument("--device-id", type=str, default="simdet", help="Device ID for the simulator")
parser.add_argument("--detector-id", type=str, default="xspdsim", help="Detector ID for the simulator")
parser.add_argument("--data-port-id", type=str, default="port-1", help="Data port ID")
parser.add_argument("--frame-width", type=int, default=516, help="Width of generated frames")
parser.add_argument("--frame-height", type=int, default=1716, help="Height of generated frames")
parser.add_argument("--data-port", type=int, default=4300, help="ZMQ data port to bind for frames")
parser.add_argument("--host", type=str, default="0.0.0.0", help="Host address for the web server")
parser.add_argument("--port", type=int, default=8000, help="Port number for the web server")
...
def set_var(path: str, value: str) -> Dict[str, Any]:
	...
	if path.startswith(args.detector_id + "/"):
		...
		if key == "thresholds":
			...
			return {"thresholds": str(thresholds)}  # driver expects string of JSON array
		# Coerce numeric types where appropriate
		if key in {"beam_energy", "shutter_time"}:
			detector_variables[key] = float(value)
		elif key in {"bit_depth", "compression_level", "n_frames", "summed_frames", "roi_rows"}:
...
def main():
	print(f"Starting XSPD simulator REST API on {args.host}:{args.port}, device-id={args.device_id}, detector-id={args.detector_id}")
